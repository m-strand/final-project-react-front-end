import React, {useState} from React.js;


function Header () {
    const [isHovering, setHover] = useState("false");
    function handleHoverToggle () {
        setHover(!isHovering);
    }

    return (
        <div className="header-component">
            <img className="logo" src="data:image/png;base64," />
            <div className="searchbar">
                <button className="btn filterbtn">Anywhere</button>
                <button className="btn filterbtn">Any Week</button>
                <button className="btn guests">Add Guests</button>
                <button className="btn material-icons searchbtn">search</button>
            </div>
            <div className="user-info">
                <p className={isHovering ? "hoverHeader btn hostbtn": "btn hostbtn"} onMouseEnter={handleHoverToggle} onMouseLeave={handleHoverToggle}>Become a Host</p>
                <button className={isHovering ? " hoverHeader material-icons btn langbtn": "material-icons btn langbtn"}>language</button>
                <button className={isHovering ? "btn hoverHeader2 usermenu": "btn usermenu"}>
                    <i className="material-icons user-icons">menu</i>
                    <i className="material-icons user-icons">person</i>
                </button> 
                {menuUp ? <HeaderDrop/> : null}
            </div>
        </div>
    );


}
export default Header;